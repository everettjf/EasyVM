...
write_report "$candidate" custom-virgl 0 1 0.7 4.2 35 900
write_report "$baseline" apple-virtio 0 0 unavailable unavailable 20 700
sed -i '' 's/VirGL-Window-Count: 5/VirGL-Window-Count: 0/' "$baseline"
"$verifier" "$candidate" "$baseline" >/dev/null

write_report "$candidate" custom-virgl 1 1 0.7 4.2 35 900
if "$verifier" "$candidate" "$baseline" >/dev/null 2>&1; then
    echo "A report with presentation failures passed unexpectedly." >&2
    exit 1
fi

write_report "$candidate" custom-virgl 0 1 0.7 4.2 50.1 900
if "$verifier" "$candidate" "$baseline" >/dev/null 2>&1; then
...
write_report "$temporary_directory/mislabeled-baseline.txt" apple-virtio 0 0 unavailable unavailable 20 700
