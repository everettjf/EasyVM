...
class CreatePhaseNameLocationViewHandler: VMCreateStepperGuidePhaseHandler {
    ...
    // ~/EZVM Virtual Machines is used when the user never picked a custom location,
    // so choosing a directory is optional in the create guide.
    static func defaultStorageDirectory() -> URL {
        FileManager.default.homeDirectoryForCurrentUser.appending(path: "EZVM Virtual Machines")
    }
    ...
    // "EZVM Machine (macOS)" -> "EZVM Machine (macOS) 2" ... until free
    ...
    func verifyForm(context: VMCreateStepperGuidePhaseContext) -> VMOSResultVoid {
        ...
        // make sure the base directory exists (e.g. the default ~/EZVM Virtual Machines)
            ...
            Form {
                ...
                Section("Location") {
                    VStack(alignment: .leading) {
                        ...
                        HStack {
                            Text("By default machines are stored in ~/EZVM Virtual Machines. Pick another directory only if you want a different location.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                            Spacer()
                            Button {
