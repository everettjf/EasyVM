...
class LinuxVMViewController: NSViewController , VZVirtualMachineDelegate {
    ...
    private func startVM() {
        ...
        if !FileManager.default.fileExists(atPath: LinuxPath.vmBundlePath) {
            needsInstall = true
...
//                    fatalError("ISO file not selected.")
    ...
    private func createVMBundle() {
        do {
            try FileManager.default.createDirectory(atPath: LinuxPath.vmBundlePath, withIntermediateDirectories: false)
        } catch {
    ...
    private func createMainDiskImage() {
        let diskCreated = FileManager.default.createFile(atPath: LinuxPath.mainDiskImagePath, contents: nil, attributes: nil)
        if !diskCreated {
            fatalError("Failed to create the main disk image.")
        }

        guard let mainDiskFileHandle = try? FileHandle(forWritingTo: URL(fileURLWithPath: LinuxPath.mainDiskImagePath)) else {
    ...
    private func createBlockDeviceConfiguration() -> VZVirtioBlockDeviceConfiguration {
        guard let mainDiskAttachment = try? VZDiskImageStorageDeviceAttachment(url: URL(fileURLWithPath: LinuxPath.mainDiskImagePath), readOnly: false) else {
    ...
    private func createAndSaveMachineIdentifier() -> VZGenericMachineIdentifier {
        ...
        try! machineIdentifier.dataRepresentation.write(to: URL(fileURLWithPath: LinuxPath.machineIdentifierPath))
        return machineIdentifier
    }

    private func retrieveMachineIdentifier() -> VZGenericMachineIdentifier {
        // Retrieve the machine identifier.
        guard let machineIdentifierData = try? Data(contentsOf: URL(fileURLWithPath: LinuxPath.machineIdentifierPath)) else {
    ...
    private func createEFIVariableStore() -> VZEFIVariableStore {
        guard let efiVariableStore = try? VZEFIVariableStore(creatingVariableStoreAt: URL(fileURLWithPath: LinuxPath.efiVariableStorePath)) else {
    ...
    private func retrieveEFIVariableStore() -> VZEFIVariableStore {
        if !FileManager.default.fileExists(atPath: LinuxPath.efiVariableStorePath) {
            fatalError("EFI variable store does not exist.")
        }

        return VZEFIVariableStore(url: URL(fileURLWithPath: LinuxPath.efiVariableStorePath))
    }
