...
| ASIF layered snapshots | Beta | ASIF disks automatically use DiskImageKit overlay stacks; raw and legacy disks retain the APFS-clone path. Audit and restore validate every layer and ASIF header. Layered restore journals its previous state, created overlays, and commit phase. Both deterministic and separate-process `_exit` matrices cover all eight durable boundaries, proving rollback before commit and roll-forward after commit without deleting referenced layers. Recovery preserves ASIF bases and safely recognizes older backups. | Validate low-space recovery with larger images, a full host restart, compaction, and long branch chains on real ASIF workloads. |
