...
| USB hot-plug management | Beta | Runtime attach/detach, descriptor parsing, shutdown safety, signed entitlement diagnostics, late-callback rejection, distinct explicit/unexpected disconnect handling, framework-specific recovery guidance, and stale `DeviceNotFound` reconciliation are integrated. | Complete permission-revocation, sleep/wake, multi-device, and physical-device compatibility testing. |
...
| Snapshot tree and branches | Stable | Tracks parent/child history, current branch, rename, protection, restore, and manifest-backed integrity audits. Create, restore, and audit expose truthful milestone progress; cancellation is accepted before commit and cleaned transactionally, while the atomic file swap is labeled as non-cancellable “Finishing safely.” Orphan ASIF cleanup has a read-only preview, strict complete-index gate, explicit confirmation, and journaled quarantine recovery. Saved-state manifests detect an active-branch change before resume. | Add transactional replacement-image consolidation. |
...
| Bridged networking | Beta | The macOS 27 VMNet capability and external-interface selection are integrated. Framework disconnects identify the affected adapter in the VM window and offer a reattach of its original configuration. | Validate Wi-Fi/Ethernet removal, VPN changes, sleep/wake, and recovery using the signed artifact. |
...
| vmnet port forwarding | Beta | TCP/UDP rules validate nonzero ports, usable in-subnet destinations, per-network duplicates, cross-network endpoint collisions, and live host-port availability before creation. The bind probe distinguishes occupied from indeterminate endpoints and never reserves the port; VMNet creation remains authoritative. | Add live rule editing, cross-process ownership, and broader reconnect tests. |
