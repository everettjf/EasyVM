...
| ASIF layered snapshots | Beta | ASIF disks automatically use DiskImageKit overlay stacks; raw and legacy disks retain the APFS-clone path. Audit and restore validate every layer and ASIF header. Layered restore now journals its previous state, created overlays, and commit phase; interrupted recovery preserves ASIF bases, rolls back non-disk files and the snapshot head, and safely recognizes older backups. | Run process-level crash injection at every mutation, then validate low-space recovery, compaction, and long branch chains on real ASIF images. |
