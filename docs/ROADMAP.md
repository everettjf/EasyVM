...
| USB hot-plug management | Beta | Runtime attach/detach, descriptor parsing, shutdown safety, signed entitlement diagnostics, late-callback rejection, distinct explicit/unexpected disconnect handling, framework-specific recovery guidance, stale `DeviceNotFound` reconciliation, and save-state exclusion throughout in-flight controller operations are integrated. | Complete permission-revocation, sleep/wake, multi-device, and physical-device compatibility testing. |
| Custom Virtio devices | Beta | macOS 27 Linux VMs have a real virtio-gpu implementation for VirGL acceleration. The backend is availability-gated, falls back safely, disables incompatible machine-state restore, and rejects aggregate renderer allocations beyond a conservative 2 GiB per-device budget before entering C. | Maintain protocol conformance, fuzz hostile resource requests, and complete wider distro/GPU soak coverage. |
