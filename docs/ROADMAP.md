...
| ASIF layered snapshots | Beta | ASIF disks automatically use DiskImageKit overlay stacks; raw and legacy disks retain the APFS-clone path. Audit and restore validate every layer and ASIF header. Layered restore journals its previous state, created overlays, and commit phase; an eight-checkpoint deterministic interruption matrix proves rollback before commit and roll-forward after commit without deleting referenced layers. Recovery preserves ASIF bases and safely recognizes older backups. | Repeat the checkpoint matrix with process termination, then validate low-space recovery, host restart, compaction, and long branch chains on real ASIF images. |
