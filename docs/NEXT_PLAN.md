...
| Clipboard completion | Ship `spice-vdagent` in the Omarchy image, display readiness/remediation for other distros, and verify text copy in both directions. | Absence does not affect boot; bidirectional Unicode and large-text tests pass; capability is no longer labelled Partial. |
...
| --- | --- |
| DiskImageKit layered ASIF snapshots | Larger-image testing on a genuinely nearly-full volume, full-host-restart recovery, performance comparison, and export/import flattening policy. Missing, damaged, and foreign base-image startup rejection; missing active-layer rejection; deterministic pre-mutation capacity rejection; eight-boundary process termination; a real 32-layer chain; stack-order validation; and branch-aware cleanup are covered. DiskImageKit 27 exposes no public in-place compaction API, so consolidation requires a transactional replacement-image design. |
